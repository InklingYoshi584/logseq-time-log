  ...
  <div className="time-log-view">
   <div className="time-log-header">
    ...
    <div className="time-log-zoom">
     <button
      ...
      onClick={() => onHourHeightChange(Math.max(defaultHourHeight, hourHeight - Math.round(defaultHourHeight * 0.33)))}
     >
      −
     </button>
     ...
     <button
      className="time-log-zoom-btn"
      onClick={() => onHourHeightChange(Math.min(240, hourHeight + Math.round(defaultHourHeight * 0.33)))}
     >
      +
     </button>
    </div>
   ...
   {loading ? (
    ...
    <div ref={gridRef} className="time-grid-scroll"
     onWheel={(e) => {
      if (e.ctrlKey) {
       onHourHeightChange(Math.max(defaultHourHeight, Math.min(240, hourHeight + (e.deltaY > 0 ? -Math.round(defaultHourHeight * 0.33) : Math.round(defaultHourHeight * 0.33)))));
      }
     }}
    >
     <TimeGrid
